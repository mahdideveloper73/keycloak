...
echo -e "\n\nSEARCH COMPANIES: ";
http://backend-keycloak-auth:8080/auth/realms/master/example/companies
http://backend-keycloak-auth:8080/realms/master/example/companies
curl -i --request GET http://backend-keycloak-auth:8080/auth/realms/master/example/companies-auth --header "Accept: application/json" --header "Authorization: Bearer $ACCESS_TOKEN";
